              ...
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-100 flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-800">{scannedResult.doctorName}</p>
                      <p className="text-xs text-slate-500">{scannedResult.date}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => handleImport(scannedResult)}
                    className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-lg transition-colors"
                  >
                    Add to Medications
                  </button>
                </div>
                {/* You can render other details of scannedResult here */}
              </div>
            )
          }
